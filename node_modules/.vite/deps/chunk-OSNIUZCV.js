import {
  ...
  useTimeout,
  useTransitionStatus
} from "./chunk-ZSXEPXCD.js";
import {
  useValueAsRef
} from "./chunk-KOL3ANN2.js";
import {
  createChangeEventDetails,
  reason_parts_exports
} from "./chunk-ZATA4CAR.js";
import {
  ...
  isWebKit,
  useStableCallback
} from "./chunk-DOGGGHXC.js";
import {
  visuallyHidden
} from "./chunk-ZJ74NKDF.js";
import {
  useId
} from "./chunk-CS4JI7KU.js";
import {
  useIsoLayoutEffect
} from "./chunk-XU3JBUCK.js";
import {
...
var useFloatingParentNodeId = () => {
  var _a;
  return ((_a = React6.useContext(FloatingNodeContext)) == null ? void 0 : _a.id) || null;
};
var useFloatingTree = (externalTree) => {
  const contextTree = React6.useContext(FloatingTreeContext);
...
function FloatingFocusManager(props) {
  ...
  const orderRef = React7.useRef(["content"]);
  const initialFocusRef = useValueAsRef(initialFocus);
  const returnFocusRef = useValueAsRef(returnFocus);
  const openInteractionTypeRef = useValueAsRef(openInteractionType);
  const tree = useFloatingTree(externalTree);
  const portalContext = usePortalContext();
  const preventReturnFocusRef = React7.useRef(false);
  const isPointerDownRef = React7.useRef(false);
  const pointerDownOutsideRef = React7.useRef(false);
  const tabbableIndexRef = React7.useRef(-1);
  const closeTypeRef = React7.useRef("");
  const lastInteractionTypeRef = React7.useRef("");
  const beforeGuardRef = React7.useRef(null);
  const afterGuardRef = React7.useRef(null);
...
function useClick(context, props = {}) {
  ...
  const pointerTypeRef = React8.useRef(void 0);
...
function useDismiss(context, props = {}) {
  ...
  const pressStartedInsideRef = React10.useRef(false);
  const pressStartPreventedRef = React10.useRef(false);
  const suppressNextOutsideClickRef = React10.useRef(false);
  const {
...
function useTriggerRegistration(id, store) {
  const registeredElementIdRef = React11.useRef(null);
  const registeredElementRef = React11.useRef(null);
  return React11.useCallback((element) => {
    if (id === void 0) {
      return;
    }
...
function useFloating2(options = {}) {
  ...
  const [positionReference, setPositionReferenceRaw] = React21.useState(null);
  ...
  const [localDomReference, setLocalDomReference] = React21.useState(null);
  const [localFloatingElement, setLocalFloatingElement] = React21.useState(null);
...
function useRole(context, props = {}) {
  ...
  const floatingId = React24.useMemo(() => {
    var _a;
  ...
  const item = React24.useCallback(({
    active,
    selected
  }) => {
    const commonProps = {
...
use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-OSNIUZCV.js.map
